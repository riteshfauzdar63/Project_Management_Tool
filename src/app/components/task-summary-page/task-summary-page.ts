...
import {
  taskSummary,
  taskSummaryResponse,
} from '../../model/task-summary/task-summary.model';
...
export class TaskSummaryPage implements OnInit {
    ...
    this.taskService.showTaskSummary(request).subscribe({
      next: (res: taskSummaryResponse) => {
        if (res.isSuccess) {
          ...
          // this.fillteredTasks = res.summaries.map((task) => ({
          //   ...task,
  ...
  onSearch(): void {
    ...
    this.fillteredTasks = this.TaskSummary.filter(
      (task) =>
        task.assignedEmployee.toLowerCase().includes(text) &&
        (this.selectedEmployee
          ? task.assignedEmployee === this.selectedEmployee
          : true)
    );
